...
// This function ensures that the total number of blocks
// calculated from the heap is the same as the total number
// of blocks recorded in the table.
static int heap_validate_table(void* ptr, void* end,
...
// Traverses the heap's block table, seeking a sequence of
// HEAP_BLOCK_TABLE_ENTRY_FREE entries that is at least as long
// as the total_blocks parameter.
//
// An error may be returned due to external fragmentation,
// i.e. a contiguous sequence of free blocks that is large enough
// could not be found due to scattered allocation.
...
void heap_mark_blocks_taken(struct heap* heap, int start_block,
    ...
    for (int i = start_block; i <= end_block; i++) {
        ...
        if (i != end_block - 1) // next block is not end block
            entry |= HEAP_BLOCK_HAS_NEXT;
    }
}
...
// Typically used for deallocation.
