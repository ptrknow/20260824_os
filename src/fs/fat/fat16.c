...
struct fat_directory {
    ...
    int total;
    int sector_pos; // for iterating over directory items
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private,
    ...
    int root_dir_sector_pos = (primary_header->fat_copies *
        primary_header->sectors_per_fat) + primary_header->reserved_sectors;
    ...
    // total_sectors or (root_dir_size / disk->sector_size)?
...
int fat16_resolve(struct disk* disk) {
    ...
    if (diskstreamer_read(stream, &fat_private->header,
            sizeof(fat_private->header)) != ALL_OK) {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private,
            &fat_private->root_directory) != ALL_OK) {
        res = -EIO;
        goto out;
    }
